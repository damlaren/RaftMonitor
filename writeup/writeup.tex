...
As a relatively recent development, the RAFT consensus algorithm \cite{raftPaper} has many implementations at varying levels of completeness and functionality. This project aims at providing a system that can test and evaluate a RAFT implementation's protocol correctness. To do so, this system simulates a cluster on which RAFT is deployed, but logs and controls all internal communication between the RAFT nodes while also executing client applications that make requests of the RAFT cluster. This setup allows both external results-centric testing of the RAFT implementation from the client point of view, and invasive testing including reading and analyzing the internal messages sent among the RAFT nodes, node failures, network partitions, and lost messages.
